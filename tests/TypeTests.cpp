...
TEST_CASE("Type - primary template default construction", "[type]")
{
    ...
    {
        fxt::Type<int> type_tag;
        using ExtractedType = typename decltype(type_tag)::type;
        REQUIRE(std::is_same_v<ExtractedType, int>);
    }

    SECTION("default construct Type<std::string>")
    {
        fxt::Type<std::string> type_tag;
        using ExtractedType = typename decltype(type_tag)::type;
...
TEST_CASE("Type - primary template implicit conversion", "[type]")
{
    ...
    {
        auto accept_int_tag = [](fxt::Type<int>) { return true; };
        REQUIRE(accept_int_tag(42));
    }

    SECTION("implicit conversion from string")
    {
        auto accept_string_tag = [](fxt::Type<std::string>) { return true; };
...
TEST_CASE("Type - primary template copy and move semantics", "[type]")
{
    ...
    {
        fxt::Type<int> tag1;
        fxt::Type<int> tag2 = std::move(tag1);
        using T2 = typename decltype(tag2)::type;
...
TEST_CASE("Type - specialization value access", "[type]")
{
    ...
    {
        const fxt::Type<std::string, int> tagged(100);
    ...
    {
        fxt::Type<std::string, int> tagged(200);
        int& val = tagged.value();
        val = 300;
    ...
    {
        fxt::Type<std::string, std::string> tagged(std::string("movable"));
    ...
    {
        fxt::Type<std::string, int> tagged(10);
        tagged.get() = 20;
...
TEST_CASE("Type - specialization copy and move semantics", "[type]")
{
    SECTION("copy construction")
    {
        fxt::Type<std::string, int> original(42);
        fxt::Type<std::string, int> copy = original;
    ...
    SECTION("move construction")
    {
        fxt::Type<std::string, std::string> original(std::string("hello"));
        fxt::Type<std::string, std::string> moved = std::move(original);
    ...
    {
        fxt::Type<std::string, int> original(42);
        fxt::Type<std::string, int> copy(0);
        copy = original;
        REQUIRE(copy.value() == 42);
        REQUIRE(original.value() == 42);
    }
    ...
    SECTION("move assignment")
    {
        fxt::Type<std::string, std::string> original(std::string("world"));
        fxt::Type<std::string, std::string> moved(std::string(""));
...
TEST_CASE("Type - specialization implicit conversion", "[type]")
{
    ...
    {
        fxt::Type<std::string, int> tagged(100);
        int& val = tagged;
        val = 200;
    ...
    SECTION("implicit conversion in function call")
    {
        ...
        fxt::Type<std::string, int> tagged(21);
...
TEST_CASE("Type - specialization comparison operators", "[type]")
{
    ...
    {
        fxt::Type<std::string, int> a(42);
        fxt::Type<std::string, int> b(42);
        REQUIRE(a == b);
    }
    ...
    {
        fxt::Type<std::string, int> a(42);
        fxt::Type<std::string, int> b(100);
        REQUIRE(a != b);
    }

    SECTION("three-way comparison - less than")
    {
        fxt::Type<std::string, int> a(10);
        fxt::Type<std::string, int> b(20);
        REQUIRE(a < b);
        REQUIRE(a <= b);
        REQUIRE(!(a > b));
        REQUIRE(!(a >= b));
    ...
    {
        fxt::Type<std::string, int> a(50);
        fxt::Type<std::string, int> b(30);
        REQUIRE(a > b);
        REQUIRE(a >= b);
        REQUIRE(!(a < b));
        REQUIRE(!(a <= b));
    ...
    {
        fxt::Type<int, std::string> a(std::string("apple"));
        fxt::Type<int, std::string> b(std::string("banana"));
        REQUIRE(a < b);
        REQUIRE(a != b);
    }
}
...
TEST_CASE("Type - specialization constexpr support", "[type]")
{
    SECTION("constexpr construction")
    {
        constexpr fxt::Type<std::string, int> tagged(42);
        static_assert(tagged.value() == 42);
    }

    SECTION("constexpr value access")
    {
        constexpr fxt::Type<std::string, int> tagged(100);
    ...
    {
        constexpr fxt::Type<std::string, int> tagged(200);
...
TEST_CASE("Type - type_t helper alias", "[type]")
{
    ...
    {
        using TagType = fxt::Type<int>;
        using ExtractedType = fxt::type_t<TagType>;
        REQUIRE(std::is_same_v<ExtractedType, int>);
    }

    SECTION("extract type from specialization")
    {
        using TagType = fxt::Type<std::string, int>;
        using ExtractedType = fxt::type_t<TagType>;
        REQUIRE(std::is_same_v<ExtractedType, std::string>);
    }
...
TEST_CASE("Type - value_type_t helper alias", "[type]")
{
    ...
    {
        using TagType = fxt::Type<int, std::string>;
        using ValueType = fxt::value_type_t<TagType>;
    ...
    {
        using TagType = fxt::Type<char, double>;
        using ValueType = fxt::value_type_t<TagType>;
...
TEST_CASE("Type - make_type() helper for pure type tag", "[type]")
{
    SECTION("create int type tag")
    {
        ...
        REQUIRE(std::is_same_v<typename decltype(tag)::type, int>);
    ...
    SECTION("create string type tag")
    {
        ...
        REQUIRE(std::is_same_v<typename decltype(tag)::type, std::string>);
        REQUIRE(fxt::is_type_v<decltype(tag)>);
    }
    ...
    SECTION("create double type tag")
    {
        ...
        REQUIRE(std::is_same_v<typename decltype(tag)::type, double>);
        REQUIRE(fxt::is_type_v<decltype(tag)>);
    }
...
TEST_CASE("Type - is_type trait", "[type]")
{
    ...
    {
        using T = fxt::Type<std::string, int>;
        REQUIRE(fxt::is_type<T>::value);
        REQUIRE(fxt::is_type_v<T>);
    }
    ...
    {
        REQUIRE(fxt::is_type_v<fxt::Type<int>>);
        REQUIRE(fxt::is_type_v<fxt::Type<std::string>>);
        REQUIRE(fxt::is_type_v<fxt::Type<double, int>>);
        REQUIRE(fxt::is_type_v<fxt::Type<char, std::string>>);
    }
}
...
{
    int process(fxt::Type<int>, int value) { return value * 2; }
    std::string process(fxt::Type<std::string>, int value) { return std::to_string(value); }
    double process(fxt::Type<double>, int value) { return value * 1.5; }
}
...
TEST_CASE("Type - tag dispatch pattern", "[type]")
{
    ...
    {
        int result = process(fxt::Type<int>{}, 21);
        REQUIRE(result == 42);
    }

    SECTION("dispatch to string overload")
    {
        std::string result = process(fxt::Type<std::string>{}, 123);
        REQUIRE(result == "123");
    }

    SECTION("dispatch to double overload")
    {
        double result = process(fxt::Type<double>{}, 10);
...
TEST_CASE("Type - with complex types", "[type]")
{
    SECTION("Type with vector")
    {
        ...
        fxt::Type<std::string, std::vector<int>> tagged(vec);
    ...
    SECTION("Type with unique_ptr")
    {
        ...
        fxt::Type<std::string, std::unique_ptr<int>> tagged(std::move(ptr));
        REQUIRE(*tagged.value() == 42);
    }

    SECTION("Type with nested Type")
    {
        fxt::Type<int, double> inner(3.14);
        fxt::Type<std::string, fxt::Type<int, double>> outer(inner);
    ...
    SECTION("Type with custom struct")
    {
        ...
        fxt::Type<std::string, Point> tagged(Point{10, 20});
...
TEST_CASE("Type - edge cases", "[type]")
{
    ...
    {
        fxt::Type<void, int> tagged(100);
        REQUIRE(std::is_same_v<typename decltype(tagged)::type, void>);
        REQUIRE(tagged.value() == 100);
    }

    SECTION("Type with const qualified tag")
    {
        fxt::Type<const int> tag;
        REQUIRE(std::is_same_v<typename decltype(tag)::type, const int>);
    }

    SECTION("Type with pointer type")
    {
        int value = 42;
        fxt::Type<std::string, int*> tagged(&value);
        REQUIRE(*tagged.value() == 42);
    }
    ...
    SECTION("Type with reference wrapper")
    {
        ...
        fxt::Type<std::string, std::reference_wrapper<int>> tagged(ref);
...
TEST_CASE("Type - noexcept specifications", "[type]")
{
    ...
    {
        REQUIRE(noexcept(fxt::Type<int>{}));
        REQUIRE(noexcept(fxt::Type<int>(42)));
    }
